...
\section{Processes}
	\begin{questions}{Process Info}
		\begin{questionAnswer}
			...
			\qItem{Nice}{Priority level [-20 (most) - 19 (least)] \ra 0 is default. Call with: nice \opt{val} \opt{process}, reset the priority level with: renice \opt{new val} \opt{PID}}
	...
	\begin{questions}{Process Signals}
		\begin{questionAnswer}
			\qItem{Send commands}{kill PID \ra kills the process}
...
\begin{questions}{Pipes \& Redirection}
	\begin{questionAnswer}
		\qItem{Pipes}{Sends the output of one file into the input of another \ra cat \opt{filename} | grep \opt{string}}
...
\section{Strings \& Searching}
	\begin{questions}{Grep}
		\begin{enumerate}
			...
			\item grep \opt{string} \opt{filename} \ra returns the lines with the character pattern \opt{string} in file filename
			\item Follow directories: grep -r \opt{string} \opt{directory}/*
